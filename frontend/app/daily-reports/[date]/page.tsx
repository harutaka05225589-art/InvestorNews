...
export async function generateMetadata({ params }: { params: Promise<{ date: string }> }): Promise<Metadata> {
    ...
    return {
        ...
        description: `${date}の日本株市場をAIが総括。上方修正や増配など、株価を動かす重要な適時開示情報とその背景を詳しく解説します。`,
        openGraph: {
...
export default async function DailyReportPage({ params }: { params: Promise<{ date: string }> }) {
    ...
    const recent = getRecentReports(5).filter(r => r.date_str !== date);
    ...
    const jsonLd = [
        ...
        },
        {
            ...
            "itemListElement": [
                ...
                { "@type": "ListItem", "position": 2, "name": "AI相場まとめ", "item": `${BASE_URL}/daily-reports` },
                { "@type": "ListItem", "position": 3, "name": report.title, "item": `${BASE_URL}/daily-reports/${report.date_str}` }
            ]
        }
    ];
            ...
            <aside style={{ flex: '1 1 300px' }}>
                <div style={{ background: '#1e293b', padding: '1.5rem', borderRadius: '12px', border: '1px solid #334155', position: 'sticky', top: '2rem' }}>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                        {recent.map((r: any) => (
                            <li key={r.id} style={{ marginBottom: '1rem' }}>
                                <Link href={`/daily-reports/${r.date_str}`} style={{ textDecoration: 'none', display: 'block' }}>
                    ...
                    <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
                         <Link href="/daily-reports" style={{ color: '#fbbf24', fontSize: '0.9rem', textDecoration: 'none' }}>
                            過去のレポート一覧へ &rarr;
                         </Link>
                    </div>
                </div>
